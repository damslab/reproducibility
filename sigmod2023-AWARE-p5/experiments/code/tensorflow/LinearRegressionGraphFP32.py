...
def readCSV(path):
    ...
    data = tf.convert_to_tensor(df, dtype=np.float32)
    return data
...
def iteration(it, p, norm_r2, W, X, r):
    ...
    eps = tf.constant(0.00001, dtype=np.float32)
    Xp = tf.matmul(X, p)
...
def exec(X, Y):
    ...
    W = tf.zeros([tf.shape(X)[1], 1], dtype=np.float32)
    i = 0
    p = tf.matmul(X, Y, transpose_a=True)
    min1 = tf.constant(-1, dtype=np.float32)
