...
def readCSV_X():
  inp = "../datasets/KDD_recode_X.csv"
  df = pd.read_csv(inp, delimiter=",", header=None)
  res = tf.convert_to_tensor(df.values, dtype=tf.float64)
  return res;

def readCSV_y():
  inp = "../datasets/KDD_y.csv"
  df = pd.read_csv(inp, delimiter=",", header=None)
  res = tf.convert_to_tensor(df.values, dtype=tf.float64)
  return res;
...
def pca(X, k):
  ...
  #evec_dominant = eigen_vectors[:, 0:t]
