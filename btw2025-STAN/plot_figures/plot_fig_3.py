...
grouped_df = df.groupby(['tags.run_id', 'params.model.name'])["UCR Score"].mean().reset_index()
try:
...
grouped_df = pd.concat([grouped_df, pd.DataFrame({"tags.run_id": [1, 1], "params.model.name": ["STAN", "MERLIN"], "UCR Score": [stan_ucr_results.mean(), merlin_ucr_results.mean()]})])


plt.figure(figsize=(10, 5))
sns.barplot(x='params.model.name', y='UCR Score', palette = reversed_palette, data=grouped_df)
