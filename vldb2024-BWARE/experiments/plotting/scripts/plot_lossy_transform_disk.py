...
def plot(df, m):
    ...
    mx = 1000000000
    mi = 1000000000
    bar_labels = ["String", "Detected", "Morph"]
    ...
    color = ["tab:blue", "tab:brown", "tab:green"]
    ...
    for id, ax in enumerate(axi):
        ...
        # ax.plot(
        #     x[1:],
        #     np.repeat(r["FMDiskSize"][:1], len(x[1:])),
        #     color=color[0],
        #     linewidth=0.8,
        #     ls="--",
        # )
        ax.plot(x[1:], r["FMDiskSize"][1:], label="Baseline", alpha=0.6, color=color[0])
        # ax.plot(
        #     x[1:],
        #     np.repeat(r["CFCMCDDiskSize"][:1], len(x[1:])),
        #     color=color[1],
        #     linewidth=0.8,
        #     ls="--",
        # )
        ax.plot(
            x[1:], r["CFCMCDDiskSize"][1:], label="BWARE+Morph", alpha=0.6, color=color[1]
        )
        # ax.plot(
        #     x[1:],
        #     np.repeat(r["CFMCMCDDiskSize"][:1], len(x[1:])),
        #     color=color[2],
        #     linewidth=0.8,
        #     ls="dotted",
        # )
        ax.plot(
            x[1:], r["CFMCMCDDiskSize"][1:], label="AWARE", alpha=0.6, color=color[2]
        )
        ...
        #     lw=0.01,
        # )
        mx = max(max(r.FMDiskSize), mx, max(r.CFCMCDDiskSize), max(r.CFMCMCDDiskSize))
        mi = min(min(r.FMDiskSize), mi, min(r.CFCMCDDiskSize), min(r.CFMCMCDDiskSize))
    ...
    for id, ax in enumerate(axi):
        ...
        ytics = pu.set_tics_y_log10(ax, mi * 0.6, mx * 3)
        if id == 0:
    ...
    plt.subplots_adjust(
        left=0.135, right=0.99, top=0.98, bottom=0.05, wspace=0.17, hspace=0.30
    )

    plt.savefig("plotting/plots/lossy_transform_" + m + "_disk.pdf")
