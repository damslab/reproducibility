...
for benchmark in benchmarks:
    # POLAR
    path = os.getcwd() + f"/experiment-results/4_1_endtoend/{benchmark}/polar/polar-tuned-1.csv"
    ...
    # POLAR-8
    path = os.getcwd() + f"/experiment-results/4_1_endtoend/{benchmark}/polar/polar-tuned-8.csv"
    df = pd.read_csv(path, names=["name", "run", "timing"])
    df = df.groupby("name", as_index=False).median()
    polar8_timings = []
    for index, row in df.iterrows():
