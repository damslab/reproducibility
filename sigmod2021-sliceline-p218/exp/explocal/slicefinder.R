...
#doMC on several machines slower/blocking
#library("doMC")
#registerDoMC(112) # physical cores
  ...
  tpEval = TRUE, tpBlksz = 16, verbose = FALSE)
{
  ...
  vmax = max(colMaxs(X))
  for( i in 1:length(fdom) ){
    rix = rbind(rix, matrix(m+1, fdom[i], 1))
    cix = rbind(cix, as.matrix(foffb[i]+seq(1:fdom[i])))
  }
...
createAndScoreBasicSlices = function(X2, e, eAvg, minSup, alpha, verbose) {
  ...
  merr = colMaxs(X2 * (e %*% matrix(1,1,ncol(X2)))); # maximum error vector
...
################################################################################
X = as.matrix(read.csv(file = args[1]))
e = as.matrix(read.csv(file = args[2]))
blksz = as.integer(args[3])
sigma = max(ceiling(0.01 * nrow(X)), 8)

TMP = slicefinder(X=X, e=e, k=10, maxL=0, alpha=0.95, minSup=sigma, tpBlksz=blksz, verbose=TRUE);
R = TMP[["TKC"]]
print(R)