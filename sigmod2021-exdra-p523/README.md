...
Local Machine and Main machine:
...
For my system the print output looks like:
...
to generate the data you can set this in the parameters.
...
## Install SystemDS on all machines
...
ALL SCRIPTS ARE TO BE EXECUTED FROM INSIDE THE EXPERIMENTS DIRECTORY FROM NOW ON!
...
Following this it setup a python venv, and installs the required packages see requirements.txt.
Then it builds the systemds python API and install that as well.
And finally, it setup the experiments folder.
...
the basic idea is that the main machine generate the data and distribute the individual federated data partitions to
