...
def writeMtd(rows: int, cols: int, name: str):
    ...
    with io.open(data_dir + name, "w", encoding="utf-8") as f:
        f.write(json.dumps(features_mtd_mtd, ensure_ascii=False))
...
writeJson(num_examples, features, "fed_" +
          data_name + str(num_workers) + ".json", data_name)

writeMtd(num_examples, features, "fed_" +
         data_name + str(num_workers) + ".json.mtd")
