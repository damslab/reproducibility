...
def plot(df, m):
    ...
    for id, ax in enumerate(axi):
        ...
        ytics = pu.set_tics_y_log10(ax, mi * 0.6, mx * 7)
        if id == 0:
            ax.set_ylabel("Time [s]", size=8)
            ax.tick_params(axis="y", labelsize=7, pad = -0.9)
            # ax.yaxis.set_label_coords(-0.4, 0.5)
            ax.yaxis.set_label_coords(x=-0.58, y=.5)
        else:
    ...
    plt.subplots_adjust(
        left=0.22, right=0.98, top=0.76, bottom=0.16, wspace=0.17, hspace=0.30
    )
