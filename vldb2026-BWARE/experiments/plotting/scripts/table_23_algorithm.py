...
mkdir("./plotting/tables/23-algorithm")
inputFileType = (".bin", ".cla")

baseAlg = ["L2SVM_last", "L2SVM_first", "pca_first", "pca_last", "kmeans_first", "kmeans_last", "kmeans10_first", "kmeans10_last"]

this = "plotting/scripts/table_23_algorithm.py"

# experiments/results/algorithms/dams-su1
#/L2SVM_last/code/conf/ULAb16.xml/singlenode/data-adult-adult.csv.bin_code-scripts-specs-adult_l10.json.log

for m in machines:
    out = "plotting/tables/23-algorithm/" + m + ".csv"
    ...
    with open(out, "w") as cv:
        ...
        cv.write("\n")
        basis =  "results/algorithms/" + m 
        for alg in baseAlg:
            # for pi, po in enumerate(poly):
                baseAlgPath = basis + "/" + alg + "/code/conf/"
                for d in data:
                    dd = d
                    if "kdd" in d:
                        ddd = "kdd98-cup98lrn.csv"
                    elif "adult" in d:
                        ddd = "adult-adult.csv"
                    elif "criteo" in d:
