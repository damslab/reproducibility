...
poly = ["", "_poly_2", "_poly_3", "_poly_4", "_poly_5", "_poly_6", "_poly_7", "_poly_8", "_poly_9"]
# l1 l2 l3 l5 l10 l20 l40 l80 l160 l320 l640 l1280 l2560

mkdir("./plotting/tables/22-poly")
inputFileType = (".bin", ".cla")

baseAlg = ["regress_lm", "binary_lm_1", "binary_lm_last"]
...
for m in machines:
    out = "plotting/tables/22-poly/" + m + ".csv"
    print("TableScript:", "plotting/scripts/table_22_poly.py", "out:", out)
    with open(out, "w") as cv:
        ...
        cv.write("\n")
        basis =  "results/poly/" + m 
        for alg in baseAlg:
            for pi, po in enumerate(poly):
                baseAlgPath = basis + "/" + alg + po + "/code/conf/"
                for d in data:
                    dd = d
                    if "kdd" in d:
