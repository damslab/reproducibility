...
def plot(df, m):
    ...
    mx = 1.0
    mi = 1000000.0
    bar_labels = ["String", "Bin", "AWARE", "BWARE", "BWARE+Morph"]
    color = [
        "tab:red",
        "tab:blue",
        "tab:green",
    ...
    for id, ax in enumerate(axi):
        ...
        l = [
            r.FMSTE,  # Add string
            r.FMTE,
            r.FMCDTE + r.FMCDCompTime,
            # r.FCMCDSysDSTime,
            # r.CFCMDSysDSTime,
            # r.CFCMCDSysDSTime,
            # r.FCMCDTE,
            r.RCFCMCDTE,
            r.RCFCMCDTE + r.RCFCMCDCompTime,
        ]
        l = [1000 if np.isnan(x) else x for x in l]
    ...
    out = "plotting/plots/compress_transform_encode_time_TE_" + m + ".pdf"
    print("Script:", "plotting/scripts/plot_TE_size_time_TE.py", "Out:", out)
    plt.savefig(out)
