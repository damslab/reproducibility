...
machines = ["dams-su1"]
conf = ["ULAb16", "AWAb16", "MKLb16", "tf2"]

mkdir("./plotting/tables/26-wordemb")
this = "plotting/scripts/table_26_wordemb_nn.py"
for m in machines:
    out = "plotting/tables/26-wordemb/" + m + "_nn.csv"
    ...
    with open(out, "w") as cv:
        ...
        basis = "results/wordemb/emb_nn_real/" + m + "/code/conf/"
        for w in words:
            for a in abstracts:
                for c in conf:
