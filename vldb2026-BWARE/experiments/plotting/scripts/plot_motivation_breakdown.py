...
def plot(df, m):
    ...
    for id, ax in enumerate(axi):
        ...
        s =  r["FMRead"]
        ax.bar([0], s, label = "I/O", zorder = 0, color=color[0])
        ...
        ax.bar([0], s, label = "Detect", zorder = -1, color=color[1])
        s = s + r["FMApplySchema"]
        ax.bar([0], s, label = "Apply", zorder = -2, color=color[2])
        s = s + r["FMTE"]
        ax.bar([0], s, label = "Transform Encode", zorder = -3, color=color[3])

        heights.append(np.array(s)[0])
        ax.grid(True, "major", axis="y", ls="--",zorder = 10, linewidth=0.9, alpha=1.0)
...
#     ax[i].tick_params(axis="y", pad=-0.06)
    for id, ax in enumerate(axi):
        ...
        pu.set_tics_y(ax, 0, heights[id], 4, 8)
    ...
    axi[0].yaxis.set_label_coords(-0.6, 0.5)
    plt.subplots_adjust(
        left=0.07, right=0.99, top=0.7, bottom=0.04, wspace=1.1, hspace=0.30
    )

    plt.savefig("plotting/plots/micro_cost_breakdown_" + m + ".pdf")
    print("script", "plotting/scripts/plot_motivation_breakdown.py", "out", "plotting/plots/micro_cost_breakdown_" + m + ".pdf")
